...
- [~] Task: Analyze `backend/app/services/screener.py` (and related files) to identify the specific sorting/filtering logic that allows positive values to slip through.
    - [ ] Review `get_top_movers` or equivalent function.
    - [ ] Identify where the "Losers" filter is applied (API side vs. Local side).
